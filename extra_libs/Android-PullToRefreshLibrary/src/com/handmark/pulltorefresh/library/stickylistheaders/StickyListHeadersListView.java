...
public class StickyListHeadersListView extends ListView {
	...
	private void measureHeader() {
		...
		if (params == null) {
			mHeader.setLayoutParams(new MarginLayoutParams(
	...
	private void scrollChanged(int reportedFirstVisibleItem) {
		...
		if (childCount != 0) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (viewToWatch == null
						|| (!viewToWatchIsFooter && (viewToWatch instanceof WrapperView) &&
						!((WrapperView) viewToWatch).hasHeader())
						|| ((childIsFooter || (viewToWatch instanceof WrapperView)) &&
